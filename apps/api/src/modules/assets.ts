import { BadRequestException, Body, ConflictException, Controller, Delete, Get, Injectable, NotFoundException, Param, Patch, Post, Req, Res, UploadedFile, UseInterceptors } from '@nestjs/common'
...
const quotaSchema=z.object({quotaBytes:z.number().int().min(1024*1024).max(100*1024*1024*1024*1024)}).strict()
...
export class AssetService{
  ...
  async list(workspaceId:string,userId:string){
    ...
    const [workspace]=await this.db.client<{quotaBytes:number}[]>`SELECT asset_quota_bytes::float8 AS "quotaBytes" FROM workspaces WHERE id=${workspaceId}`
    return{assets:assets.map(({storageKey:_,...item})=>item),usage:{usedBytes,quotaBytes:workspace?.quotaBytes??ASSET_WORKSPACE_QUOTA_BYTES}}
  }
  async updateQuota(workspaceId:string,userId:string,quotaBytes:number){await this.workspaces.role(workspaceId,userId,'workspace.manage');const [usage]=await this.db.client<{usedBytes:number}[]>`SELECT coalesce(sum(size_bytes),0)::float8 AS "usedBytes" FROM assets WHERE workspace_id=${workspaceId}`;if((usage?.usedBytes??0)>quotaBytes)throw new BadRequestException({code:'ASSET_QUOTA_BELOW_USAGE',message:'容量上限不能低于当前已用空间'});const [row]=await this.db.client<{quotaBytes:number}[]>`UPDATE workspaces SET asset_quota_bytes=${quotaBytes} WHERE id=${workspaceId} RETURNING asset_quota_bytes::float8 AS "quotaBytes"`;return{quotaBytes:row!.quotaBytes,usedBytes:usage?.usedBytes??0}}
  async upload(workspaceId:string,userId:string,file?:Upload){
    ...
    try{return await this.db.client.begin(async sql=>{
      ...
      const [usage]=await sql<{usedBytes:number;quotaBytes:number}[]>`SELECT coalesce(sum(a.size_bytes),0)::float8 AS "usedBytes",w.asset_quota_bytes::float8 AS "quotaBytes" FROM workspaces w LEFT JOIN assets a ON a.workspace_id=w.id WHERE w.id=${workspaceId} GROUP BY w.id`
      if((usage?.usedBytes??0)+file.buffer.length>(usage?.quotaBytes??ASSET_WORKSPACE_QUOTA_BYTES))throw new BadRequestException({code:'ASSET_QUOTA_EXCEEDED',message:'工作区静态资源空间不足，请联系管理员调整容量上限'})
...
export class AssetController{
  ...
  @Patch('quota')updateQuota(@Req() req:AppRequest,@Param('workspaceId') workspaceId:string,@Body() body:unknown){return this.assets.updateQuota(workspaceId,req.user!.id,parse(quotaSchema,body).quotaBytes)}
