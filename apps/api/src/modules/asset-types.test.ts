...
const zip=(files:Record<string,string>)=>Buffer.from(zipSync(Object.fromEntries(Object.entries(files).map(([name,value])=>[name,Buffer.from(value)]))))
...
describe('asset type detection',()=>{
  it('accepts common documents and design deliverables',()=>{
    ...
    expect(detectAssetType('flow.drawio',Buffer.from('<mxfile/>'))).toBe('application/vnd.jgraph.mxfile')
    expect(detectAssetType('design.docx',zip({'word/document.xml':'<w:document/>','[Content_Types].xml':'types'}))).toBe('application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    expect(detectAssetType('prototype.sketch',zip({'document.json':'{}','meta.json':'{}'}))).toBe('application/vnd.sketch')
    expect(detectAssetType('mockup.psd',Buffer.from('8BPSmockup'))).toBe('image/vnd.adobe.photoshop')
  })
