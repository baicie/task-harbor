  ...
  async restore(data:Uint8Array,userId:string){
    ...
    try{return await this.db.client.begin(async sql=>{
      ...
      for(const project of snapshot.projects){
        ...
        for(const [index,column] of project.columns.entries()){const last=project.columns.length-1,state=column.stateType??(index===0?'BACKLOG':index===last?'DONE':index===last-1?'REVIEW':'ACTIVE'),key=column.key??(index===0?'BACKLOG':index===last?'DONE':index===last-1?'REVIEW':index===1?'ACTIVE':`ACTIVE_${index+1}`);const [next]=await sql<{id:string}[]>`INSERT INTO board_columns(workspace_id,project_id,key,name,color,state_type,position) VALUES(${workspace!.id},${created!.id},${key},${column.name},${column.color},${state},${column.position}) RETURNING id`;columnIds.set(column.sourceId,next!.id)}
        if(project.transitions?.length){for(const transition of project.transitions)await sql`INSERT INTO workflow_transitions(workspace_id,project_id,from_column_id,to_column_id,name,bug_name,requires_comment,position) VALUES(${workspace!.id},${created!.id},${columnIds.get(transition.fromColumnSourceId)!},${columnIds.get(transition.toColumnSourceId)!},${transition.name},${transition.bugName},${transition.requiresComment},${transition.position})`}
        else for(const [index,column] of project.columns.entries()){const next=project.columns[index+1];if(next)await sql`INSERT INTO workflow_transitions(workspace_id,project_id,from_column_id,to_column_id,name,bug_name,position) VALUES(${workspace!.id},${created!.id},${columnIds.get(column.sourceId)!},${columnIds.get(next.sourceId)!},'下一状态','下一状态',${(index+1)*1000})`;if((column.stateType==='REVIEW'||(!column.stateType&&index===project.columns.length-2))&&index>0){const previous=project.columns[index-1]!;await sql`INSERT INTO workflow_transitions(workspace_id,project_id,from_column_id,to_column_id,name,bug_name,requires_comment,position) VALUES(${workspace!.id},${created!.id},${columnIds.get(column.sourceId)!},${columnIds.get(previous.sourceId)!},'驳回修改','验证失败',true,${(index+1)*1000+500})`}}
