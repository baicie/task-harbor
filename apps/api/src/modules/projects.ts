...
export class ProjectService {
  ...
  async create(workspaceId:string,userId:string,input:ReturnType<typeof projectSchema.parse>) { await this.workspaces.role(workspaceId,userId,'project.create'); return this.db.client.begin(async sql => { const [project]=await sql<{id:string;name:string;code:string}[]>`INSERT INTO projects(workspace_id,name,code,description,color,lead_id) VALUES(${workspaceId},${input.name},${input.code},${input.description},${input.color},${userId}) RETURNING id,name,code`; const defaults=[['待处理','#84908b'],['进行中','#2367d1'],['待验收','#d5792a'],['已完成','#27825a']]; for (const [index,item] of defaults.entries()) await sql`INSERT INTO board_columns(workspace_id,project_id,name,color,position) VALUES(${workspaceId},${project!.id},${item![0]!},${item![1]!},${(index+1)*1000})`; await sql`INSERT INTO project_members(workspace_id,project_id,user_id) VALUES(${workspaceId},${project!.id},${userId})`; await sql`INSERT INTO audit_logs(workspace_id,actor_id,action,entity_type,entity_id,request_id,after_data) VALUES(${workspaceId},${userId},'project.created','project',${project!.id},'system',${JSON.stringify(input)}::jsonb)`; return project }) }
}
