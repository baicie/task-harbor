import { Body, Controller, Delete, Get, Injectable, Param, Patch, Post, Req } from '@nestjs/common'
...
export class ProjectService {
  ...
  async create(workspaceId:string,userId:string,input:ReturnType<typeof projectSchema.parse>) { await this.workspaces.role(workspaceId,userId,'project.create'); return this.db.client.begin(async sql => { const [project]=await sql<{id:string;name:string;code:string}[]>`INSERT INTO projects(workspace_id,name,code,description,color,lead_id,workflow_template) VALUES(${workspaceId},${input.name},${input.code},${input.description},${input.color},${userId},${input.workflowTemplate}) RETURNING id,name,code`; await installWorkflow(sql,workspaceId,project!.id,input.workflowTemplate); await sql`INSERT INTO project_members(workspace_id,project_id,user_id) VALUES(${workspaceId},${project!.id},${userId})`; await sql`INSERT INTO audit_logs(workspace_id,actor_id,action,entity_type,entity_id,request_id,after_data) VALUES(${workspaceId},${userId},'project.created','project',${project!.id},'system',${JSON.stringify(input)}::jsonb)`; return project }) }
}
...
export class ProjectController {
  ...
  @Delete(':projectId') async remove(@Req() req:AppRequest,@Param('workspaceId') workspaceId:string,@Param('projectId') projectId:string) { await this.workspaces.role(workspaceId,req.user!.id,'project.delete'); await this.db.client.begin(async sql=>{await sql`UPDATE projects SET deleted_at=now(),deleted_by=${req.user!.id} WHERE id=${projectId} AND workspace_id=${workspaceId} AND deleted_at IS NULL`;await sql`UPDATE tasks SET deleted_at=now(),deleted_by=${req.user!.id} WHERE project_id=${projectId} AND workspace_id=${workspaceId} AND deleted_at IS NULL`;await sql`INSERT INTO audit_logs(workspace_id,actor_id,action,entity_type,entity_id,request_id) VALUES(${workspaceId},${req.user!.id},'project.deleted','project',${projectId},${req.requestId})`});return {ok:true} }
}
