...
export class SessionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const rows = await this.db.client<{ id: string; email: string; name: string; csrf_token: string }[]>`
    ...
    if (!['GET','HEAD','OPTIONS'].includes(req.method) && req.headers['x-csrf-token'] !== user.csrf_token) throw new UnauthorizedException({ code: 'CSRF_INVALID', message: '安全令牌无效' })
