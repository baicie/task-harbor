...
function TaskCalendar({tasks,en,onTaskOpen}:{tasks:Task[];en:boolean;onTaskOpen:(task:Task)=>void}) {
  ...
  return <div className="task-calendar"><div className="calendar-toolbar"><strong>{monthLabel}</strong><span><Button variant="outline" size="sm" onClick={()=>setCursor(new Date(today.getFullYear(),today.getMonth(),1))}>{en?'Today':'今天'}</Button><Button variant="outline" size="icon-sm" aria-label={en?'Previous month':'上个月'} onClick={()=>setCursor(new Date(year,month-1,1))}><ChevronLeft/></Button><Button variant="outline" size="icon-sm" aria-label={en?'Next month':'下个月'} onClick={()=>setCursor(new Date(year,month+1,1))}><ChevronRight/></Button></span></div><div className="calendar-scroll"><div className="calendar-grid"><div className="calendar-weekdays">{weekdays.map(day=><span key={day}>{day}</span>)}</div><div className="calendar-days">{cells.map(date=>{const key=format(date),dayTasks=byDate.get(key)??[],outside=date.getMonth()!==month;return <div className={`calendar-day ${outside?'is-outside':''} ${key===todayKey?'is-today':''}`} key={key}><time dateTime={key}>{date.getDate()}</time><div className="calendar-events">{dayTasks.slice(0,2).map(task=><Button type="button" variant="ghost" className={`calendar-event calendar-event--${task.kind.toLowerCase()}`} key={task.id} onClick={()=>onTaskOpen(task)}><span>{task.title}</span></Button>)}{dayTasks.length>2?<small>+{dayTasks.length-2} {en?'more':'项'}</small>:null}</div></div>})}</div></div></div>{datedTasks.length===0?<p className="calendar-empty">{en?'Add a due date to a task and it will appear here.':'在任务详情中设置截止日期后，任务会显示在这里。'}</p>:null}</div>
}
